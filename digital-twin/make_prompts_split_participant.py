...
def build_prompts_for_participants(data, personas_by_id, keep_participants):
    dims = ['content', 'design', 'coping', 'quitting']
    buckets = {d: [] for d in dims}
    for item in data:
        pid = item['response_id']
        if pid not in keep_participants:
            continue
...
def main():
    ...
    ap.add_argument('--out-dir', type=str, default='digital-twin/prompts_test_participants')
    ...
    uniq_participants = sorted({d['response_id'] for d in data})
    train_p, test_p = train_test_split(uniq_participants, test_size=args.test_size, random_state=args.seed)

    # Save split lists
    os.makedirs(args.splits_dir, exist_ok=True)
    with open(os.path.join(args.splits_dir, 'participants_train.txt'), 'w') as f:
        f.write('\n'.join(train_p))
    with open(os.path.join(args.splits_dir, 'participants_test.txt'), 'w') as f:
        f.write('\n'.join(test_p))

    test_part = set(test_p)

    def filtered_persona(pid, persona):
        # For unseen (test) participants, drop history entirely to avoid leakage
        if pid in test_part:
            p = dict(persona)
            p['history'] = {'rated_messages': []}
            return p
        return persona

    # Build test-only prompts for unseen participants with filtered persona history
    dims = ['content', 'design', 'coping', 'quitting']
    buckets = {d: [] for d in dims}
    for item in data:
        pid = item['response_id']
        if pid not in test_part:
            continue
        persona = personas_by_id.get(pid)
        if not persona:
            continue
        persona_json = json.dumps(filtered_persona(pid, persona), ensure_ascii=False)
        msg = item.get('input_message')
        img_id = item.get('metadata', {}).get('Image ID')
        image_tags = f"ImageID:{img_id}" if img_id is not None else None
        for dim in dims:
    ...
    print(f"Wrote participant-split test-only prompts to {args.out_dir}")
