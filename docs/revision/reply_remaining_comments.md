...
- Source checked: `docs/revision/amiajnl-2026-019201_Proof_hi.pdf`.
...
Evidence: `.scripts/legacy/run_full_manuscript_pipeline.sh`,
`../run_generic_dt7030_pipeline.sh`, and
`../run_generic_dt7030_variance_pipeline.sh` call
`../analysis/legacy/main_eval.py` with `--mode text-only`; image payloads are
...
- The generic/personalized method bar grids were removed from
  `figures` so they are not mistaken for replacement main figures.
