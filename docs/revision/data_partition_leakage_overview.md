...
This document consolidates the partition logic, leakage audit, and draft response prose for reviewer comments that touch on splits, exemplars, history-test overlap, and what each model could "see." Pulls together evidence already present in `figures/label_leakage_analysis.csv` and the canonical splits in `data/splits/canonical/`.
...
## 2. Leakage audit summary (from `figures/label_leakage_analysis.csv`)
...
- An Appendix A4 table reproducing the contents of `figures/label_leakage_analysis.csv`.
