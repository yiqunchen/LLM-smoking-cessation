...
`dt10` was built (`analysis/build_dt10_splits.py`) specifically to **fix**
...
Recomputed directly from `figures/lc_dt10_ensemble_k7.csv` (means across
...
Generator: `analysis/make_figure2_dt10.py`. Outputs:
...
- Split structure: `data/splits/canonical/{train,test}_dt10_k{1,3,5,7}.json`
- Supervised + LLM-PP + (dropped) anchors: `figures/lc_dt10_ensemble_k{1,3,7}.csv`
- Supervised-only all blocks: `figures/lc_dt10_rf.csv`
- Generic zero/few-shot: `figures/lc_dt10_generic_llm.csv`
- LLM digital-twin per model: `figures/lc_dt10_llmdt.csv`
