...
current all-model bar figures, and writes revision-native outputs:
  ...
  - figures/accuracy_confidence_intervals.png/pdf
  - figures/accuracy_confidence_intervals.md

The intent is to answer the reviewer request for confidence intervals of the
reported accuracy rates with a direct artifact under figures.

Usage:
    uv run python analysis/legacy/accuracy_ci_summary.py
    uv run python analysis/legacy/accuracy_ci_summary.py --plot-only
"""
