...
Usage:
    uv run python analysis/legacy/create_comprehensive_figures.py
    uv run python analysis/legacy/create_comprehensive_figures.py --plot-only
"""
...
EMBEDDING_BASELINE_PATH = 'archive/results/embedding_results_test30/results.json'
...
MODEL_CONFIGS = {
    'gpt-4o-mini': {'dir': 'results/manuscript/gpt-4o-mini', 'display': 'GPT-4o-mini', 'color': '#0173B2'},  # Blue
    'gpt-5': {'dir': 'results/manuscript/gpt-5', 'display': 'GPT-5', 'color': '#DE8F05'},  # Orange
    'deepseek_deepseek-r1-0528': {'dir': 'results/manuscript/deepseek_deepseek-r1-0528', 'display': 'DeepSeek-R1', 'color': '#029E73'},  # Green
    'x-ai_grok-4-fast': {'dir': 'results/manuscript/x-ai_grok-4-fast', 'display': 'Grok-4-Fast', 'color': '#CC78BC'},  # Purple
    'gemini-2.5-pro': {'dir': 'results/manuscript/gemini-2.5-pro', 'display': 'Gemini-2.5-Pro', 'color': '#CA9161'},  # Brown
...
def consistent_result_path(model_id: str, model_cfg: dict, method_cfg: dict, split: str) -> str | None:
    ...
    if method_cfg['key'] == 'hybrid':
        ...
        hybrid_dir = f'results/manuscript/hybrid_{model_safe}'
...
def collect_consistent_split_results(split: str = CONSISTENT_SPLIT) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    baseline_path = os.path.join('figures', 'history_supervised_baselines.csv')
...
def main():
    ...
    if df is None or len(df) == 0:
        ...
        print("   and figures/history_supervised_baselines.csv.")
        return
