...
Outputs:
    figures/lc_rf_vs_llm_h2h.md          (human-readable tables)
    figures/lc_rf_vs_llm_h2h.csv         (long-form, all metrics)

Usage:
    uv run python analysis/legacy/lc_rf_vs_llm_h2h_table.py
"""
...
def render_md(long_df: pd.DataFrame) -> str:
    ...
    out.append("- Per-split per-method per-domain metrics: "
               "`figures/lc_rf_vs_llm.csv`\n"
               "- Generation: `analysis/legacy/lc_rf_vs_llm.py` "
               "(RF training + LLM aggregation), "
               "`analysis/legacy/lc_rf_vs_llm_h2h_table.py` (this table).\n")
    out.append(
        "Notes:\n"
