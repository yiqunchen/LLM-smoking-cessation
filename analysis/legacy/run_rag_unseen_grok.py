...
Setup
-----
  - Train fold:  data/splits/canonical/train_participant_3070.json
                 (90 seen users, 275 labelled messages)
  - Test fold:   data/splits/canonical/test_participant_3070.json
...
Output
------
  results/manuscript/x-ai_grok-4-fast/rag_unseen_p3070_k{K}.json
  Same JSON schema as digital_twin_*.json files.

Usage
-----
  OPENROUTER_API_KEY=... uv run python analysis/legacy/run_rag_unseen_grok.py [--limit N] [--k K]
"""
...
CANONICAL_DIR = PROJECT_ROOT / "data/splits" / "canonical"
OUT_DIR = PROJECT_ROOT / "results/manuscript/x-ai_grok-4-fast"
...
async def run(k: int, limit: int | None, max_concurrent: int,
    ...
    if restrict_to_existing_llm:
        existing_path = (PROJECT_ROOT / "results/manuscript/x-ai_grok-4-fast"
