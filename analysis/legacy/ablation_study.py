...
Run:
    uv run python analysis/legacy/ablation_study.py           # dry-run (default)
    uv run python analysis/legacy/ablation_study.py --run     # actual API execution (future)
"""
...
REVISION_FIGURES_DIR = os.path.join(PROJECT_ROOT, 'figures')
...
def prepare_test_data_with_profiles() -> list:
    ...
    train_path = os.path.join(PROJECT_ROOT, 'data/splits', 'canonical', 'train_dt10_k7.json')
    test_path = os.path.join(PROJECT_ROOT, 'data/splits', 'canonical', 'test_dt10_k7.json')
...
def main():
    ...
    if ablation_rows:
        ...
        status_path = write_ablation_status(
            ...
            "- Prompt constructors in `analysis/legacy/ablation_study.py`\n"
            "- Canonical PP splits in `data/splits/canonical/`\n\n"
