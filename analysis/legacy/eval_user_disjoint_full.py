...
Outputs
-------
    figures/unseen_participant/headline_3070full.{md,csv}
    figures/unseen_participant/headline_1090full.{md,csv}
    figures/unseen_participant/headline_combined.md
    figures/unseen_participant/zero_shot_footnote.md   (subset where zero-shot exists)
"""
...
OUT_DIR = PROJECT_ROOT / "figures" / "unseen_participant"
OUT_DIR.mkdir(parents=True, exist_ok=True)
GROK_DIR = PROJECT_ROOT / "results/manuscript/x-ai_grok-4-fast"
...
SPLITS = [
    {"tag": "3070full", "label": "30/70 user-disjoint (90 train / 211 test users)",
     "train": "data/splits/canonical/train_user_disjoint_3070_full.json",
     "test":  "data/splits/canonical/test_user_disjoint_3070_full.json",
     "rag_llm": "results/manuscript/x-ai_grok-4-fast/rag_unseen_3070full_k3.json"},
    {"tag": "1090full", "label": "10/90 user-disjoint (30 train / 271 test users)",
     "train": "data/splits/canonical/train_user_disjoint_1090_full.json",
     "test":  "data/splits/canonical/test_user_disjoint_1090_full.json",
     "rag_llm": "results/manuscript/x-ai_grok-4-fast/rag_unseen_1090full_k3.json"},
]
...
def step2_run_rf(split: dict) -> Path:
    ...
    cmd = [
        sys.executable,
        str(PROJECT_ROOT / "analysis" / "legacy" / "rag_features_rf_unseen.py"),
...
def step4_zero_shot_footnote():
    ...
    md = ["# Zero-shot footnote: subset where zero-shot Grok already exists\n",
          ...
          "`results/manuscript/x-ai_grok-4-fast/`.  For each split, we grade ",
