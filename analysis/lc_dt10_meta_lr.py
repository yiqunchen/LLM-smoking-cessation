...
Outputs:
    figures/lc_dt10_meta_lr_k{k}.csv
    (and rows are also appended to `lc_dt10_ensemble_k{k}.csv` under the
    label `Stacked LR` so the figure picks them up.)

Usage:
    uv run python analysis/lc_dt10_meta_lr.py            # k=1, 3, 7
    uv run python analysis/lc_dt10_meta_lr.py 3          # single k
"""
...
def grok_path(k: int) -> str:
    return os.path.join(
        PROJECT_ROOT, "results/manuscript/x-ai_grok-4-fast",
