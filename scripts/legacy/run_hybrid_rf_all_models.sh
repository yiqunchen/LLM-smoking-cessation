...
echo "This legacy 70/30 hybrid pipeline is retired. Use dt10 canonical splits; see docs/DATA_POLICY.md."
exit 2
...
TRAIN_FILE="data/splits/canonical/train_digital_twin_7030.json"
TEST_FILE="data/splits/canonical/test_digital_twin_7030.json"
RF_PREDICTIONS_FILE="results/manuscript/hybrid_rf_history/rf_predictions_history_features.json"
MAX_CONCURRENT=5

# Check if RF predictions exist
if [ ! -f "$RF_PREDICTIONS_FILE" ]; then
    echo "❌ ERROR: RF predictions not found!"
    echo "Please run: python analysis/legacy/build_hybrid_history_rf_predictions.py"
    exit 1
fi
...
for model_spec in "${MODELS[@]}"; do
    ...
    OUTPUT_DIR="results/manuscript/hybrid_${model_safe}"
    ...
    HYBRID_RF_PREDICTIONS="$RF_PREDICTIONS_FILE" python analysis/legacy/main_eval.py \
        --mode text-only \
        --model "$model" \
        --provider "$provider" \
...
for model_spec in "${MODELS[@]}"; do
    ...
    echo "  - results/manuscript/hybrid_${model_safe}/"
done
echo ""
echo "Next steps:"
echo "  1. Run analysis: python analysis/legacy/analyze_manuscript_results.py"
echo "  2. Generate figures: python analysis/legacy/create_comprehensive_figures.py"
echo ""
