...
if [ "$MODE" = "watch-and-launch" ]; then
    ...
    uv run python analysis/legacy/apples_to_apples_variance.py subset --domain "$DOMAIN" --wait --save "$SUBSET_FILE" | tee -a "$LOG_FILE"
    ...
    shift 2 || true
    PATTERN="${1:-results/manuscript/*/generic_llm_*_dt7030_${DOMAIN}_rep*.json}"
    uv run python analysis/legacy/apples_to_apples_variance.py summarize --pattern "$PATTERN" --domain "$DOMAIN" | tee -a "$LOG_FILE"
else
