...
#
# Usage:
#   bash scripts/legacy/run_full_manuscript_pipeline.sh <model_name> [<provider>]
#
# Examples:
#   bash scripts/legacy/run_full_manuscript_pipeline.sh gpt-4o-mini
#   bash scripts/legacy/run_full_manuscript_pipeline.sh gpt-4o
#   bash scripts/legacy/run_full_manuscript_pipeline.sh deepseek/deepseek-r1 openrouter
#   bash scripts/legacy/run_full_manuscript_pipeline.sh x-ai/grok-2-1212 openrouter
#   bash scripts/legacy/run_full_manuscript_pipeline.sh gemini-2.0-flash-exp gemini
#
# Features:
...
echo "This legacy 70/30 pipeline is retired. Use the dt10 canonical splits; see docs/DATA_POLICY.md."
exit 2
...
if [ -z "$1" ]; then
    ...
    echo ""
    echo "Usage: bash scripts/legacy/run_full_manuscript_pipeline.sh <model_name> [<provider>]"
    echo ""
    echo "Examples:"
    echo "  bash scripts/legacy/run_full_manuscript_pipeline.sh gpt-4o-mini"
    echo "  bash scripts/legacy/run_full_manuscript_pipeline.sh gpt-4o"
    echo "  bash scripts/legacy/run_full_manuscript_pipeline.sh deepseek/deepseek-r1 openrouter"
    echo "  bash scripts/legacy/run_full_manuscript_pipeline.sh x-ai/grok-2-1212 openrouter"
    echo "  bash scripts/legacy/run_full_manuscript_pipeline.sh gemini-2.0-flash-exp gemini"
    exit 1
fi
...
# Configuration
CANONICAL_SPLITS_DIR="data/splits/canonical"
OUTPUT_DIR="results/manuscript/${MODEL_SAFE}"
...
if [ -f "$CANONICAL_SPLITS_DIR/test_participant_7030.json" ]; then
    ...
    run_with_logging \
        "python analysis/legacy/create_canonical_splits.py" \
...
run_with_logging \
    "python analysis/legacy/main_eval.py \
        --mode text-only \
...
if [ -f "$OUTPUT_DIR/generic_llm_5_continuous.json" ]; then
    run_with_logging \
        "python analysis/legacy/compare_llm_vs_individual.py \
...
run_with_logging \
    "python analysis/legacy/analyze_manuscript_results.py" \
