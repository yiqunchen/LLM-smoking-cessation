...
EVAL_DIR="${ROOT_DIR}/digital-twin/eval_participant_split"
...
if [ -f "${ROOT_DIR}/digital-twin/personas/personas.json" ]; then
  ...
  python "${ROOT_DIR}/digital-twin/build_personas.py" --input "${ROOT_DIR}/data/processed_llm_data.json" --out-dir "${ROOT_DIR}/digital-twin/personas"
fi

echo "[2/5] Creating participant-based split and test-only prompts (skip if present)"
PROMPTS_DIR="${ROOT_DIR}/digital-twin/prompts_test_participants"
if ls "${PROMPTS_DIR}"/*.jsonl >/dev/null 2>&1; then
  echo "  prompts_test_participants/*.jsonl found; skipping make_prompts_split_participant.py"
else
  python "${ROOT_DIR}/digital-twin/make_prompts_split_participant.py" \
...
OUT_CSV="${ROOT_DIR}/digital-twin/predictions_part_${MODEL_TAG}_temp_${TEMP}.csv"

echo "[3/5] Running async twin inference on TEST prompts (unseen participants; model=${MODEL}, temp=${TEMP}, concurrency=${CONCURRENCY})"
...
echo "[4/5] Evaluating twins against ground truth (participant split)"
python "${ROOT_DIR}/digital-twin/evaluate_twins.py" \
  --pred-csv "${OUT_CSV}" \
  --data-json "${ROOT_DIR}/data/processed_llm_data.json" \
  --out-dir "${EVAL_DIR}"

echo "[5/5] Residual fusion diagnostic (baseline + embedding residuals; participant split)"
...
echo "\nDone (participant split). Key artifacts:"
